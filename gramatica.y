...
%token CTE ID IF ELSE END_IF WHILE PRINT INT BEGIN END ULONG DO UNTIL CLASS EXTENDS TO_ULONG VOID CADENA  
%token '<=' '>=' '<>' ':=' '=='
...
declaracionClase : CLASS ID BEGIN lista_declaraciones_dentro_de_clase END						{if (!a.TablaSimbolo.get($2.sval).estaDeclarado()){
																									asignarUso($2.sval, "CLASE");;
																									setDeclarado($2.sval);}
																								 else
				 ...
				 | CLASS ID BEGIN  END															{if (!a.TablaSimbolo.get($2.sval).estaDeclarado()){
																									asignarUso($2.sval, "CLASE");
																									setDeclarado($2.sval);}
																								 else 
																									yyerror( "El nombre de la clase ya fue utilizado");
				 ...
				 | CLASS ID EXTENDS lista_herencias BEGIN lista_declaraciones_dentro_de_clase END   {if (!a.TablaSimbolo.get($2.sval).estaDeclarado()){
																										asignarUso($2.sval, "CLASE");;
																										setDeclarado($2.sval);
																										setHerencias($2.sval);}
																									else
				 ...
				 | CLASS ID EXTENDS ID BEGIN lista_declaraciones_dentro_de_clase END			{if (!a.TablaSimbolo.get($2.sval).estaDeclarado())
																									if (a.TablaSimbolo.get($4.sval).estaDeclarado()){
																										asignarUso($2.sval, "CLASE");;
																										setDeclarado($2.sval);
																										setHerencia($2.sval, $4.sval);}
																									else
																										yyerror( "La clase "+$4.sval+" no esta declarada");
																								 else 
																									yyerror( "El nombre de la clase ya fue utilizado");
      																							 yaCambie = false;
      																							 ambiente = "GLOBAL";
				 ...
				 | CLASS ID EXTENDS lista_herencias BEGIN  END    								{if (!a.TablaSimbolo.get($2.sval).estaDeclarado()){
																									asignarUso($2.sval, "CLASE");
																									setDeclarado($2.sval);
																									setHerencias($2.sval);}
																								 else 
				 ...
				 | CLASS ID EXTENDS ID BEGIN  END												{if (!a.TablaSimbolo.get($2.sval).estaDeclarado())
																									if (a.TablaSimbolo.get($4.sval).estaDeclarado()){
																										asignarUso($2.sval, "CLASE");;
																										setDeclarado($2.sval);
																										setHerencia($2.sval, $4.sval);}
				 ...
				 | CLASS  BEGIN lista_declaraciones_dentro_de_clase END 						{yyerror($1, "Falta nombre de la Clase");}
				 | CLASS ID EXTENDS  BEGIN lista_declaraciones_dentro_de_clase END 				{yyerror($1, "Falta lista de clases padre");
      																							 yaCambie = false;
      																							 ambiente = "GLOBAL";
																								 asignarAmbiente($2.sval);}
				 | CLASS  EXTENDS lista_de_ID BEGIN lista_declaraciones_dentro_de_clase END 	{yyerror("Falta nombre de la Clase");}

;

declaracionObjeto : ID ID ';'																	{if(a.TablaSimbolo.get($1.sval).estaDeclarado())
																									if (!a.TablaSimbolo.get($2.sval).estaDeclarado()){
																										asignarUso($2.sval, "OBJETO");
				  ...
				  | ID lista_de_ID ';'															{if(a.TablaSimbolo.get($1.sval).estaDeclarado()){
																									AsignarInfo($1.sval, "OBJETO");}
																								 else 
																									yyerror("La clase "+ $1.sval + " no esta declarada");}
;
declaracionAtributo : tipo lista_de_ID ';'														{
																								 AsignarInfo($1.sval, "ATRIBUTO");}
																				 
					| tipo ID ';'																{if (!a.TablaSimbolo.get($2.sval).estaDeclarado()){
																									AsignarTipo($1.sval, $2.sval);
																									asignarUso($2.sval,"ATRIBUTO");
																									setDeclarado($2.sval);
																									asignarAmbiente($2.sval);}
																								 else 
																									yyerror("El nombre del Atributo "+$2.sval+" ya fue utilizado");}
;
...
identificador     :    ID  { Simbolo s = a.TablaSimbolo.get($1.sval);                                                         $$.obj = s;}
				  |    ID '.' ID    		{
											  Simbolo attrib = a.TablaSimbolo.get($3.sval);
											  Simbolo objeto = a.TablaSimbolo.get($1.sval);
